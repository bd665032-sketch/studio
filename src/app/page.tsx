...
export default function MemberPortal() {
      ...
      try {
        const response = await fetch("https://api.web3forms.com/submit", {
          ...
          body: JSON.stringify({
            ...
            message: `আপনার রেজিস্ট্রেশন ওটিপি কোড: ${code}`,
  ...
  if (userLoading || txLoading) {
    return (
      <div className="min-h-screen bg-[#1A1140] flex items-center justify-center">
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#1A1140] flex flex-col items-center justify-center p-4 font-body relative overflow-hidden">
        ...
        <div className="w-full max-w-[400px] bg-white/95 backdrop-blur-3xl rounded-[45px] shadow-2xl p-8 relative z-10">
          <div className="text-center mb-10">
            <div className="w-24 h-24 mx-auto bg-white p-2 rounded-full shadow-lg border-2 border-blue-50 mb-4 flex items-center justify-center overflow-hidden">
              {settings?.logo ? (
          ...
          <div className="flex bg-slate-100 p-1.5 rounded-[28px] mb-8 shadow-inner">
            <button onClick={() => { setIsLogin(true); setAuthStep("login-register"); }} className={`flex-1 py-4 rounded-[24px] text-[11px] font-black uppercase transition-all ${isLogin ? 'bg-[#1E3A8A] text-white shadow-lg' : 'text-slate-400'}`}>লগইন</button>
            <button onClick={() => { setIsLogin(false); setAuthStep("login-register"); }} className={`flex-1 py-4 rounded-[24px] text-[11px] font-black uppercase transition-all ${!isLogin ? 'bg-[#1E3A8A] text-white shadow-lg' : 'text-slate-400'}`}>রেজিস্ট্রেশন</button>
          </div>
            ...
            <form onSubmit={handleStartAuth} className="space-y-4">
              {!isLogin && (
                <Select onValueChange={(v) => setAuthData({ ...authData, fullName: v })}>
                  <SelectTrigger className="h-16 rounded-[24px] bg-slate-50 border-none font-black text-[#1E3A8A] px-6">
              ...
              <Button type="submit" disabled={authLoading} className="w-full h-16 rounded-[24px] bg-[#1E3A8A] text-white font-black text-xs uppercase tracking-widest shadow-xl mt-4">
                {authLoading ? <Loader2 className="animate-spin" /> : (isLogin ? "SECURE LOGIN" : "SEND OTP CODE")}
              </Button>
            </form>
          ) : (
            <form onSubmit={handleVerifyAndRegister} className="space-y-6 animate-in slide-in-from-right duration-500">
              <button type="button" onClick={() => setAuthStep("login-register")} className="flex items-center gap-2 text-[#1E3A8A] font-black text-xs uppercase mb-4"><ChevronLeft className="w-4 h-4" /> ফিরে যান</button>
              <div className="relative">
                <KeyRound className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                <Input type="text" maxLength={6} placeholder="কোড দিন" required value={otpInput} onChange={(e) => setOtpInput(e.target.value)} className="h-20 pl-16 rounded-[24px] bg-slate-50 border-none font-black text-3xl tracking-[0.5em] text-center" />
              </div>
      ...
      {activeTab === "home" && (
        <main className="flex-1 overflow-y-auto pb-32 animate-in fade-in duration-700">
          <div className="relative pt-16 pb-28 px-8 text-center border-b-[12px] border-[#D4AF37]/10">
            <button onClick={() => { if(auth) signOut(auth); }} className="absolute top-8 right-8 bg-white/5 p-3 rounded-full text-white/40"><LogOut className="w-5 h-5" /></button>
            <div className="flex justify-center mb-8">
              <div className="w-32 h-32 rounded-full border-[6px] border-[#D4AF37]/20 p-1 bg-white shadow-2xl overflow-hidden flex items-center justify-center">
                {settings?.logo ? <img src={settings.logo} className="w-full h-full object-cover" /> : <span className="text-5xl text-[#1E3A8A] font-black italic">MG</span>}
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 mt-4 text-[#D4AF37]/50 text-[10px] font-bold uppercase tracking-[0.2em]"><ShieldCheck className="w-4 h-4" /> Device Secured</div>
          </div>
      ...
      {activeTab === "history" && (
        <main className="flex-1 overflow-y-auto pb-32 px-6 pt-16">
          <div className="flex items-center justify-between mb-8 px-2">
            <h2 className="font-black text-2xl flex items-center gap-4 uppercase tracking-tight"><History className="text-[#D4AF37] w-8 h-8" /> জমার রিপোর্ট</h2>
            <div className="flex gap-2">
              <Select value={selectedSummaryMonth} onValueChange={setSelectedSummaryMonth}>
                ...
                <SelectContent className="bg-[#2D1B69] border-white/10 shadow-2xl rounded-2xl z-[500]">
                  {months.map(m => <SelectItem key={m} value={m} className="font-black py-4 text-white uppercase text-xs focus:bg-white/10 focus:text-white">{m}</SelectItem>)}
                </SelectContent>
              </Select>
              <Button onClick={handleDownloadPDF} variant="ghost" className="bg-[#D4AF37] text-black h-12 rounded-2xl font-black px-5 shadow-lg"><Download className="w-5 h-5" /></Button>
            </div>
          </div>
          <div className="bg-gradient-to-b from-[#2D1B69] to-[#1A1140] p-12 rounded-[45px] border border-white/10 text-center shadow-2xl mb-10">
            <p className="text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.3em] mb-3">{selectedSummaryMonth === "All" ? "মোট জমার পরিমাণ" : `${selectedSummaryMonth.toUpperCase()} মাসের মোট জমা`}</p>
          ...
          <div className="space-y-5">
            {filteredTransactions.map((t: any) => (
              <div key={t.id} className="bg-white/5 p-8 rounded-[40px] flex items-center justify-between border border-white/5 shadow-2xl backdrop-blur-sm">
                ...
                <button onClick={() => handleDelete(t.id)} className="p-4 bg-red-500/10 text-red-500 rounded-full hover:bg-red-500 hover:text-white transition-all"><Trash2 className="w-6 h-6" /></button>
              </div>
            ))}
          </div>
        </main>
